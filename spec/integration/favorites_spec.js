...
describe("routes : favorites", () => {
  ...
  describe("signed in user favoriting and unfavoriting", () => {
    ...
    describe("POST /topics/:topicId/posts/:postId/favorites/create", () => {
      ...
      it("should return an array when calling getFavorites()", done => {
        const options = {
          url: `${base}${this.topic.id}/posts/${this.post.id}/favorites/create`,
        };
    ...
    describe("POST /topics/:topicId/posts/:postId/favorites/:id/destroy", () => {
      ...
      it("should delete a favorite", done => {
        ...
        request.post(options, (err, res, body) => {
          this.post.getFavorites()
          .then( favorites => {
            console.log("inside delete describe: ", favorites.length);
            const favorite = favorites[0];
            favCountBeforeDelete = favorites.length;
            console.log("favCountBeforeDelete: ", favCountBeforeDelete);
            expect(favCountBeforeDelete).toBe(2); //one we added plus one at addition
            request.post(`${base}${this.topic.id}/posts/${this.post.id}/favorites/${favorite.id}/destroy`,
              (err, res, body) => {
                this.post.getFavorites()
                .then( favorites => {
                  expect(favorites.length).toBe(1); //one that was made upon creation
                  done();
                })
                .catch( err => {
                  console.log(err);
  ...
  }); //end of signed in user suite

}) // end of route test suite
