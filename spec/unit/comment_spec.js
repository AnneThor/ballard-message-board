...
describe("Comment", () => {
  ...
  describe("#create()", () => {
    ...
    it("should not create a comment with missing body, assigned post or user", done => {
      ...
      .then( comment => {
        //the code here will never run because such a comment cannot be created
        done();
      })
      .catch( err => {
  ...
  describe("#setUser()", () => {
    it("should associate a comment and a user together", done => {
      ...
      .then( newUser => {
        expect(this.comment.userId).toBe(this.user.id); //confirming it belongs to prev user
  ...
  describe("#setPost()", () => {
    it("should associate a post and a comment together", done => {
      ...
      .then( newPost => {
        expect(this.comment.postId).toBe(this.post.id); //confirm comment associated with different post
